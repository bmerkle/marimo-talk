...
def _(alt, arr, df_orig, mat, mo, np, pd):
    ...
    chart_sim = alt.Chart(df_sim).mark_point().encode(x="x", y="y") + alt.Chart(
        df_orig
    ).mark_point(color="gray").encode(x="x", y="y")
...
def _(Matrix, mo, np, pd):
    pca_mat = mo.ui.anywidget(
        Matrix(np.random.normal(0, 1, size=(3, 2)), step=0.1)
    )
...
def _(color_picker, mo):
    mo.vstack(
        ...
        """),
            mo.Html(
                f'<p style="color: {color_picker.color}">Change my color!</p>'
            ),
            mo.hstack(
                [
                    color_picker,
                    mo.md(
                        f"You selected {color_picker.value['color']} which is {color_picker.rgb} in RGB values."
                    ),
                ]
            ),
        ]
...
def _(np, pd):
    x_orig = np.random.multivariate_normal(
        np.array([0, 0]), np.array([[1, 0], [0, 1]]), 2500
    )
